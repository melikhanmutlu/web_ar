document.addEventListener('DOMContentLoaded', function() {
    ...
    modelViewer.addEventListener('load', () => {
        ...
        // Convert dimensions to meters (assuming model units are in meters)
        const dimensions = {
            x: size.x,
            y: size.y,
            z: size.z
        ...
        document.getElementById('model-width').textContent = dimensions.x.toFixed(2) + ' m';
        document.getElementById('model-height').textContent = dimensions.y.toFixed(2) + ' m';
        document.getElementById('model-depth').textContent = dimensions.z.toFixed(2) + ' m';

        // Calculate scale factor if any dimension exceeds MAX_DIMENSION
        const MAX_DIMENSION = 0.5; // 50 cm = 0.5 meters
        const maxCurrentDimension = Math.max(dimensions.x, dimensions.y, dimensions.z);
        
        if (maxCurrentDimension > MAX_DIMENSION) {
            const scaleFactor = MAX_DIMENSION / maxCurrentDimension;
            
            // Apply uniform scaling
            modelViewer.scale = `${scaleFactor} ${scaleFactor} ${scaleFactor}`;
            
            // Update scale input controls
            document.getElementById('scale-x').value = scaleFactor;
            document.getElementById('scale-y').value = scaleFactor;
            document.getElementById('scale-z').value = scaleFactor;
